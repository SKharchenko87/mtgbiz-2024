...
// handleConnections1 - обработчик для первой задачи
func handleConnections1(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("Ошибка при получения соединения до WebSocket: %v", err)
		return
	}
	defer ws.Close()
	...
	if err != nil {
		log.Printf("Ошибка при получения соединения до БД: %v", err)
		return
	}
	defer db.Close()
...
// handleConnections2 - обработчик для второй задачи
